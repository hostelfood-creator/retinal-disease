...
export const DetailModal: React.FC<DetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-8 animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-6xl max-h-[95vh] overflow-hidden bg-white shadow-2xl animate-in zoom-in-95 slide-in-from-bottom-8 duration-300">
        {/* Header */}
        <div className="sticky top-0 z-10 flex items-center justify-between border-b-2 border-slate-100 bg-white px-10 py-6">
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center bg-slate-900">
              <Pulse weight="bold" className="h-6 w-6 text-white" />
            </div>
            <div>
              <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight font-heading">Diagnostic Report</h3>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mt-0.5 font-technical">ASSET: {prediction.filename}</p>
            </div>
          </div>
          <button 
            onClick={onClose} 
            className="flex h-12 w-12 items-center justify-center bg-slate-50 text-slate-400 hover:bg-slate-900 hover:text-white transition-all active:scale-95"
          >
            <X weight="bold" size={20} />
          </button>
        </div>

        <div className="overflow-y-auto p-12 custom-scrollbar max-h-[calc(95vh-160px)] font-sans">
          <div className="grid grid-cols-1 gap-16 lg:grid-cols-12">
            ...
            <div className="lg:col-span-5 flex flex-col gap-10">
              ...
              <div className="clinical-card p-10 bg-slate-50/30 border-2 border-slate-100 shadow-none">
                <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-6 font-heading">ASSESSMENT SUMMARY</h4>
                <div className="flex items-baseline justify-between mb-8 pb-8 border-b border-slate-100">
                  <div className="flex flex-col">
                    <span className="text-[10px] font-bold text-slate-400 uppercase mb-2 font-heading">Diagnosis</span>
                    <span className="text-2xl font-black uppercase tracking-widest text-slate-900 font-heading">{prediction.diagnosis}</span>
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="text-[10px] font-bold text-slate-400 uppercase mb-2 font-heading">Confidence</span>
                    <span className="text-4xl font-black text-slate-900 font-technical">{conf.toFixed(1)}%</span>
                  </div>
                </div>
                
                <div className="flex gap-6 items-start">
                   <div className="h-12 w-12 bg-slate-900 flex items-center justify-center shrink-0">
                      <Info weight="bold" className="h-6 w-6 text-white" />
                   </div>
                   <div>
            ...
            <div className="lg:col-span-7 flex flex-col gap-10">
              <div className="clinical-card p-10 shadow-none border-2 border-slate-100 bg-white">
              ...
              <div className="clinical-card p-10 shadow-none border-2 border-slate-100 flex flex-col items-center justify-center bg-slate-50/20">
                <div className="w-full flex items-center gap-3 mb-8 pb-4 border-b border-slate-100">
        ...
        {/* Footer */}
        <div className="sticky bottom-0 z-10 border-t-2 border-slate-100 bg-slate-50 px-12 py-8 flex justify-end gap-6 font-heading">
           <button 
             onClick={onDownloadPDF} 
             className="flex items-center gap-3 bg-slate-900 px-10 py-5 text-[11px] font-black uppercase tracking-[0.2em] text-white hover:bg-black transition-all"
           >
